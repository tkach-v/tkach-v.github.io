...
import {YtLikedVideoItem} from "@/src/types";

const YtLikedVideoCard: React.FC<{ item: YtLikedVideoItem }> = ({item}) => (
  <>
    <img src={item.thumbnailUrl} alt={item.title} className="w-full rounded-lg mb-3"/>
    <h4 className="font-semibold text-white mb-1">{item.title}</h4>
    <p className="text-sm text-gray-400 mb-2">{item.channelTitle}</p>
    <p className="text-xs text-gray-500">{new Date(item.videoPublishedAt).toLocaleDateString()}</p>
    {item.description && (
